...
export default function BlogPostsUpdateForm(props) {
  const {
    id: idProp,
    blogPosts: blogPostsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = blogPostsRecord
      ? { ...initialValues, ...blogPostsRecord }
      : initialValues;
    setPostTitle(cleanValues.postTitle);
    setPostCategory(cleanValues.postCategory);
    setPostAuthor(cleanValues.postAuthor);
    setPostBody(cleanValues.postBody);
    setPostImage(cleanValues.postImage);
    setErrors({});
  };
  const [blogPostsRecord, setBlogPostsRecord] =
    React.useState(blogPostsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getBlogPosts.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getBlogPosts
        : blogPostsModelProp;
      setBlogPostsRecord(record);
    };
    queryData();
  }, [idProp, blogPostsModelProp]);
  React.useEffect(resetStateValues, [blogPostsRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateBlogPosts.replaceAll("__typename", ""),
            variables: {
              input: {
                id: blogPostsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "BlogPostsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Post title"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || blogPostsModelProp)}
